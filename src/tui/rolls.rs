...
impl StatusApp {
    ...
    fn render_status_bar(&self, f: &mut Frame, area: Rect) {
        ...
        // `[v]erify` sits with graduate and promote because that is what it
        // checks the way to; `[b]ump` moves down to make room, which also puts it
        // next to the other keys that are answers to a failed verify.
        let roll_line =
            Line::from(" [c]reate   [i]ntegrate   [v]erify   [G]raduate   [m] promote   [u]pdate");
        let extra_line =
            Line::from(" [b]ump   [d]elete   [x] prune   [t]idy   [PgUp/PgDn/End] scroll output");
        f.render_widget(
